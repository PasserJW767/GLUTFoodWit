...
//        设置透明状态栏
    ...
    private void checkPermission(Activity activity) {//开启本地的照片读取与写入权限
        ...
        try {
            //检测是否有写的权限
            int permission = ActivityCompat.checkSelfPermission(Voucher.this,
                    "android.permission.WRITE_EXTERNAL_STORAGE");
            if (permission != PackageManager.PERMISSION_GRANTED) {
                // 没有写的权限，去申请写的权限，会弹出对话框
                ActivityCompat.requestPermissions(Voucher.this, PERMISSIONS_STORAGE, REQUEST_EXTERNAL_STORAGE);
            }
